...
export class ComputadorController {
  constructor(private computadorService: ComputadorService) {}
    ...
    @Post('create')
    create(@Body() body: { nome: string; cor: string; dataDeFabricacao: number }) {
      return this.computadorService.create(body.nome, body.cor, body.dataDeFabricacao);
    }
    ...
    addPeriferico(@Param('nome') nome: string, @Body() body: { periferico: string }) {
      return this.computadorService.addPeriferico(nome, body.periferico);
    }
}
